...
StitchOctreeNode* BackRecurse(OctreeNode* node, StitchOctreeNode* existing, MeshBuilder& builder)
{
	...
	if (node->type == NODE_INTERNAL)
	{
		for (uint8 i = 0; i < 4; i++)
		{
			...
			existing->children[idx] = BackRecurse(node->children[idx], existing->children[idx], builder);
		}
	}
	else if (!did_exist)
	{
...
StitchOctreeNode* FrontRecurse(OctreeNode* node, StitchOctreeNode* existing, MeshBuilder& builder)
{
	...
	if (node->type == NODE_INTERNAL)
	{
		for (uint8 i = 0; i < 4; i++)
		{
			...
			existing->children[idx] = FrontRecurse(node->children[idx], existing->children[idx], builder);
		}
	}
	else if (!did_exist)
	{
...
StitchOctreeNode* TopRecurse(OctreeNode* node, StitchOctreeNode* existing, MeshBuilder& builder)
{
	...
	if (node->type == NODE_INTERNAL)
	{
		for (uint8 i = 0; i < 4; i++)
		{
			...
			existing->children[idx] = TopRecurse(node->children[idx], existing->children[idx], builder);
		}
	}
	else if (!did_exist)
	{
...
StitchOctreeNode* BottomRecurse(OctreeNode* node, StitchOctreeNode* existing, MeshBuilder& builder)
{
	...
	if (node->type == NODE_INTERNAL)
	{
		for (uint8 i = 0; i < 4; i++)
		{
			...
			existing->children[idx] = BottomRecurse(node->children[idx], existing->children[idx], builder);
		}
	}
	else if (!did_exist)
	{
